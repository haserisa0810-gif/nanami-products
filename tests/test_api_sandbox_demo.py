...
class ApiSandboxDemoTest(unittest.TestCase):
    ...
    def test_api_sandbox_contains_data_creation_ui(self) -> None:
        ...
        self.assertIn("AIに渡せる占術データを3分で作る", html)
        self.assertIn("APIキー実計算版", html)
        self.assertIn("有料版を利用するには", html)
        self.assertIn("https://nanami-astro.stores.jp/?category_id=6a065b1efe1d6b05e4df2f4c", html)
        self.assertIn("STORESでAPIを購入", html)
        self.assertIn("購入済みの方: APIキー発行", html)
        self.assertIn("占術データを生成する", html)
        self.assertIn("AI貼り付け用", html)
