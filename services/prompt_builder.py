...
上のURLがこの鑑定データ専用のURLの場合、保存済みの元YAMLが自動で読み込まれます。AI自身がURLを参照できなくても、ユーザーはリンクを直接開けるため、URLをそのまま案内してください。

汎用ACGアプリが開いてYAML入力欄が表示された場合は、納品ページまたは保存済みファイルから元の占術YAMLを貼り付けてもらってください。AIが会話内の長いYAMLを再掲すると、省略や書き換えが起きる可能性があるため、AIによるYAMLの再掲・再構成はしないでください。

対象地点を検索した後、ACGアプリから出力されたYAMLを会話へ貼り付けてもらうよう案内してください。
